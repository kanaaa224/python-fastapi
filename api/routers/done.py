...
async def mark_task_as_done(task_id: int, db: AsyncSession = Depends(get_database)):
...
async def unmark_task_as_done(task_id: int, db: AsyncSession = Depends(get_database)):
    done = await done_crud.get_done(db, task_id=task_id)
