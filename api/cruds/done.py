...
async def create_done(db: AsyncSession, task_id: int) -> task_model.Done:
    ...
    result = await db.execute(select(task_model.Done).filter(task_model.Done.id == task_id))

    if result.first():
        raise HTTPException(status_code=400, detail="Task already marked as done")
